(* CSE341, Programming Languages, Homework 7, hw7.sml (see also Ruby code) *)
...
fun intersect (v1,v2) =
	...
	(case intersect(two_points_to_line seg, v2) of
	  ...
	  | Point(x0,y0) => (* see if the point is within the segment bounds *)
		...
		val (x1,y1,x2,y2) = seg
	    in
		if inbetween(x0,x2,x1) andalso inbetween(y0,y2,y1)
		then Point(x0,y0)
		else NoPoints
	    end
	  ...
	  | LineSegment seg2 =>
	     ...
	     *)
	    let
		val (x1end,y1end,x1start,y1start) = seg
		val (x2end,y2end,x2start,y2start) = seg2
	    in
		if real_close(x1start,x1end)
		then (* the segments are on a vertical line *)
		    (* let segment a start at or below start of segment b *)
		    let
			val ((aXend,aYend,aXstart,aYstart),
			     (bXend,bYend,bXstart,bYstart)) = if y1start < y2start
							      then (seg,seg2)
							      else (seg2,seg)
		    in
		    ...
		    let
			val ((aXend,aYend,aXstart,aYstart),
			     (bXend,bYend,bXstart,bYstart)) = if x1start < x2start
							      then (seg,seg2)
							      else (seg2,seg)
		    in
