...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if isinbetween(x, seg.x2, seg.x1) and isinbetween(y, seg.y2, seg.y1)
      Point.new(x, y)
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1, x2)
      aXend, aYend, aXstart, aYstart, bXend, bYend, bXstart, bYstart =
                              y2 < seg.y2 ? seg1 + seg2 : seg2 + seg1
      if real_close(aYend, bYstart)
        Point.new(aXend, aYend)
      elsif aYend < bYstart
        NoPoints.new
      elsif aYend > bYend
        LineSegment.new(bXend, bYend, bXstart, bYstart)
      else
        LineSegment.new(aXend, aYend, bXstart, bYstart)
      end
    else
      aXend, aYend, aXstart, aYstart, bXend, bYend, bXstart, bYstart =
                              x2 < seg.x2 ? seg1 + seg2 : seg2 + seg1
