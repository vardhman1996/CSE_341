...
d = d.preprocess_prog
if not (d.x2 == -THREE and d.y2 == -FOUR and d.x1 == ONE and d.y1 == TWO)
	puts "LineSegment preprocess_prog should make x1 and y1"
	puts "on the left of x2 and y2"
end

d3 = d.shift(THREE,FIVE)
if not (d3.x2 == ZERO and d3.y2 == ONE and d3.x1 == FOUR and d3.y1 == SEVEN)
	puts "LineSegment shift not working properly"
end

d4 = d.intersect(LineSegment.new(ONE,TWO,-THREE,-FOUR))
if not (((d4.is_a? LineSegment)) and d4.x2 == -THREE and d4.y2 == -FOUR and d4.x1 == ONE and d4.y1 == TWO)
	puts "LineSegment intersect not working properly"
end
d5 = d.intersect(LineSegment.new(FOUR,FIVE,TWO,THREE).preprocess_prog)
if not ((d5.is_a? NoPoints))
	puts "LineSegment intersect not working properly"
end

#Intersect Tests
i = Intersect.new(LineSegment.new(THREE,FOUR,-ONE,-TWO), LineSegment.new(-ONE,-TWO,THREE,FOUR))
i1 = i.preprocess_prog.eval_prog([])
if not (i1.x2 == -ONE and i1.y2 == -TWO and i1.x1 == THREE and i1.y1 == FOUR)
...
#Let Tests
l = Let.new("a", LineSegment.new(THREE,FOUR,-ONE,-TWO),
             Intersect.new(Var.new("a"),LineSegment.new(-ONE,-TWO,THREE,FOUR)))
l1 = l.preprocess_prog.eval_prog([])
if not (l1.x2 == -ONE and l1.y2 == -TWO and l1.x1 == THREE and l1.y1 == FOUR)
...
if not (l2.x2 == -ONE and l2.y2 == -TWO and l2.x1 == THREE and l2.y1 == FOUR)
	puts "Let eval_prog should evaluate e2 after adding [s, e1] to the environment"
end
...
if not (s1.x2 == TWO and s1.y2 == THREE and s1.x1 == SIX and s1.y1 == 9)
